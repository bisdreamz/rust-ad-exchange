...
#[serde(default)]
pub struct Endpoint {
    /// Stable unique identifier — generated by the UI on creation.
    /// Falls back to `name` when empty (legacy docs without an id).
    pub id: String,
    pub enabled: bool,
    pub name: String,
    pub url: String,
...
impl Endpoint {
    /// Returns the stable identity key — `id` if set, otherwise `name` for
    /// backwards compatibility with legacy endpoint docs that predate the id field.
