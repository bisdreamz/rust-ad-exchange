...
pub fn validate_local_id(local_id: &str) -> bool {
    local_id.trim().starts_with(constants::CONST_REX_USER_PREFIX)
}

/// Generate a local buyer id value with our
/// ['REX_USER_PREFIX'] platform prefix
pub fn generate_local_id() -> String {
    format!("{}{}", constants::CONST_REX_USER_PREFIX, Uuid::new_v4())
}
...
pub fn build_kind_pixel(sync: &SyncConfig, local_uid: &String, local_uid_macro: &str) -> String {
    let safe_url = sync.url
        .replace('"', "&quot;")
        .replace(local_uid_macro, local_uid);

    match sync.kind {
        SyncKind::Image => {
            format!("<img height=\"1\" width=\"1\" style='display: none;' src=\"{}\" />", safe_url)
        }
        SyncKind::Iframe => {
            format!("<iframe height=\"1\" width=\"1\" style='display: none;' src=\"{}\"></iframe>", safe_url)
        }
    }
}
...
pub fn generate_sync_iframe_html(local_uid: &String, bidders: Vec<Arc<Bidder>>, pub_sync: Option<SyncConfig>) -> String {
    let mut pixels = Vec::with_capacity(bidders.len() + 1);
    let target_local_uid_macro = usersync::constants::CONST_REX_LOCAL_ID_MACRO;
    ...
    for bidder in bidders.iter() {
        ...
        if bidder_sync.url.trim().is_empty() {
            warn!("Syncing configured for bidder {} but pixel url empty!", bidder.name);
            continue;
        }

        pixels.push(build_kind_pixel(bidder_sync, local_uid, target_local_uid_macro));
