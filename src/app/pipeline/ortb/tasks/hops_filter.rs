...
impl BlockingTask<AuctionContext, Error> for SchainHopsGlobalFilter {
    fn run(&self, context: &AuctionContext) -> Result<(), Error> {
        let span = child_span_info!("schain_hops_filter_task").entered();
        ...
        if req.source.is_none() {
            ...
            span.record("filter_filter_len", 0);
        ...
        if schain.nodes.len() > self.hop_limit as usize {
            ...
            span.record("filter_filter_len", schain.nodes.len());

            let brs = BidResponseState::NoBidReason {
                reqid: req.id.clone(),
                nbr: rtb::spec::nobidreason::DAILY_DOMAIN_CAP,
            ...
            bail!("Hops filter limit exceeded: {} > {}", schain.nodes.len(), self.hop_limit);
        }

        debug!("Passed schain length filter ({} <= {})", schain.nodes.len(), self.hop_limit);
