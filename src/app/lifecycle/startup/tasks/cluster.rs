...
impl AsyncTask<StartupContext, Error> for ClusterDiscoveryTask {
    ...
    async fn run(&self, context: &StartupContext) -> Result<(), Error> {
        let cluster_cfg = &context.config.get()
        ...
        let cluster_discovery: Arc<dyn ClusterDiscovery> = match cluster_cfg {
            ...
            ClusterConfig::K8s => {
                let k8s = KubeClusterDiscovery::start().await
                    .map_err(|e| anyhow!("Failed to connect to kube cluster API: {}", e.to_string()))?;

                info!("Connected to k8s for cluster discovery, current size {}",
                    k8s.cluster_size());
        ...
        context.cluster_manager.set(cluster_discovery)
